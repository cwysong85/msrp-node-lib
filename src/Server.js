...
module.exports = function (MsrpSdk) {
  ...
  Server.prototype.start = function (callback) {
    const server = this;
    server.server
      .listen(MsrpSdk.Config.port, MsrpSdk.Config.host, function () {
        const serverAddress = server.server.address();
        MsrpSdk.Logger.info(
          `[MSRP Server] MSRP TCP server listening on ${serverAddress.address}:${serverAddress.port}`
        );
      ...
      .on("connection", function (socket) {
        MsrpSdk.Logger.debug(
          `[MSRP Server] Socket connected. Remote address: ${socket.remoteAddress}:${socket.remotePort}`
        );
        ...
        if (session) {
          try {
            ...
            MsrpSdk.Logger.error(
              `[MSRP Server] Error setting socket for session ${session.sid}: ${error}`
            );
          }
        } else {
          ...
          MsrpSdk.Logger.warn(
            `[MSRP Server] No session found for remote address ${socket.remoteAddress}:${socket.remotePort}. Waiting for session setup...`
          );
          ...
          setTimeout(() => {
            ...
            if (socketIndex !== -1) {
              MsrpSdk.Logger.warn(
                `[MSRP Server] Dangling socket timeout. Socket with address ${socket.remoteAddress}:${socket.remotePort} has not been assigned to any Session. Closing socket...`
              );
              socket.end();
